...
deps:
  - { concept: /excerpts/the-loop-exposed-its-own-ceiling.md, rel: refines }
...
Verified against the 36Kr page; as-reported caveat. Reframes the loop→graph transition as a legibility trade; the exit-report argument itemizes the same tax from practice.

# Relations

- **refines** → [The loop exposed its own ceiling](../excerpts/the-loop-exposed-its-own-ceiling.md)
