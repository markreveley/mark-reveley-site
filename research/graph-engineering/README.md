...
| Field | On | Meaning |
|---|---|---|
| `subtype` | Excerpt | The unit's epistemic role. Enum: `claim` \| `problem` \| `solution` \| `observation` \| `inference` \| `prescription` (definitions below) |
...
- **claim** — an assertion about what something is or how things are. Definitions are claims (tagged `definition`).
...
Known friction (flagged for the maintainer): the enum has no `question` (the seed thread's OP speaks in questions — mapped here to `problem`) and no `definition` (mapped to `claim` + tag `definition`). Several quotes are dual-role (a prescription usually implies a claim); the primary subtype is a judgment call recorded per-concept, with the secondary role noted in `# Analysis`.
...
- **Discipline**: `loop-engineering`, `graph-engineering`, `standards`, `tooling`, `multi-agent`, `history`, `academic`
...
- Verified verbatim: all excerpts from 24 of 28 sources.
...
2. **Enum coverage.** Should the enum grow `question` (seed-thread OP) and `definition` (currently `claim`+tag)? Or is coercion into the six given values intended?
